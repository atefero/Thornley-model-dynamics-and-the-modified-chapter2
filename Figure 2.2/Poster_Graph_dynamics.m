function Poster_Graph_dynamics
...
for i = 1:size(Mx,1)      % Different Sigma PIs
    ...
    figure(1)
    subplot(2,3,c);
    plot(t1, M,'Color','#f03b20','LineWidth', 2.5) %[0 0.8 0.4]  [0.6 0 1]: Purple '#e6550d'
    hold on
    %grid on
    xlabel('\bf{Time (Day)}', 'fontsize' , 10)
    set (gca, 'fontsize', 15)
    ylabel('\bf{Total Mass (kg)}', 'fontsize' , 20)
    %legend( 'Original Model', 'FontSize', 14 ,'Location','best')
    pubfig

%     % Remove figure background color
%     set(gcf, 'Color', 'none');
% 
%     % Remove axes background color
%     set(gca, 'Color', 'none');
% 
% % Save the figure as a PNG file with a transparent background
% print(gcf, 'myFigure.png', '-dpng', '-r300', '-opengl');
    ...
    set (gca, 'fontsize', 10)
    %ylabel('6 State Variables', 'fontsize' , 15)
...
%     % Remove the background color of the figure and axes
%     set(gcf, 'Color', 'none');
%     set(gca, 'Color', 'none');
% 
%     % Save the figure with a transparent background
%     exportgraphics(gcf, 'myFigure.png', 'BackgroundColor', 'none', 'Resolution', 300);
%     drawnow
